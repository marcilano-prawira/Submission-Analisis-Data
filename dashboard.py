...
st.markdown("""
Dashboard ini dikembangkan untuk memberikan visualiasi dan insight hasil analisis penyewaan sepeda berdasarkan data historis (2011-2012). 
Sehingga dapat melihat langsung: 
1. Fluktuasi tren penyewaan terhadap cuaca & musim (2011-2012).
2. Tren bulanan (2011-2012) peminjaman pelanggan Casual dan Registered.
...
def load_data():
    ...
    df['Bulan'] = df['dteday'].dt.to_period('M').astype(str)
...
# sidebar filter dan logo
st.sidebar.markdown("Analisis Data Bike Sharing (2011-2012)")

# logo (ambil dari kaggle copy link gambar)
...
st.sidebar.title("Filter rentang waktu penyewaan sepeda")
...
# fluktuasi sewa by musim dan cuaca
...
with col1:
    ...
    sns.barplot(data=musim_df, x='season', y='cnt', color='#3498db', ax=ax1a)
...
with col2:
    ...
    sns.barplot(data=cuaca_df, x='weathersit', y='cnt', color='#e74c3c', ax=ax1b)
...
st.success("""
**Insight :** Berdasarkan rentang waktu yang dipilih, terlihat jelas fluktuasi penyewaan sangat dipengearuhi oleh musim dan cuaca. 
**Rekomendasi Bisnis:** Tim operasional dapat memanfaatkan waktu saat musim sepi (seperti Winter/Spring) atau saat cuaca memburuk untuk melakukan **perawatan sepeda (maintenance) secara keseluruhan**, agar armada siap dipakai maksimal saat musim panas dan gugur.
""")

st.divider()

# tren bulanan by pelanggan casual dan registered
st.header("2. Tren Bulanan Pelanggan (Casual dan Registered)")

tren_bulanan = main_df.groupby('Bulan')[['casual', 'registered']].sum().reset_index()
tren_bulanan_melted = pd.melt(tren_bulanan, id_vars=['Bulan'], value_vars=['casual', 'registered'],
                             var_name='Tipe Pelanggan', value_name='Total Penyewaan')

fig2, ax2 = plt.subplots(figsize=(14, 6))
sns.lineplot(data=tren_bulanan_melted, x='Bulan', y='Total Penyewaan', hue='Tipe Pelanggan', 
             palette=['#66b3ff', '#ff9999'], marker='o', linewidth=2.5, ax=ax2)

ax2.set_title('Tren Bulanan Penyewaan Sepeda (Casual dan Registered)', fontsize=16)
ax2.set_xlabel('Bulan (Tahun-Bulan)', fontsize=12)
...
st.info("""
**Insight:** Pola tren bulanan di atas sangat jelas menunjukkan fluktuasi penyewaan harian oleh pelanggan member (registered) dan casual.
**Rekomendasi Bisnis:** Tim marketing dapat merencanakan **promo membership spesial (diskon pendaftaran)** kepada pelanggan casual pada bulan bulan puncak penyewaan (Di bulan liburan musim panas).
""")

st.divider()

# proposi total pelanggan casual dan registered
